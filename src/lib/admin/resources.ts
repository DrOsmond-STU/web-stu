...
export const RESOURCES: Resource[] = [
  ...
  },
  {
    ...
    labelSingular: 'Skema Sertifikasi',
    description:
      'Daftar skema sertifikasi internasional dan nasional (BNSP) yang dapat diikuti melalui kami. Tampil pada halaman Sertifikasi.',
    icon: 'badge',
    orderBy: 'sort_order, id',
    searchable: ['name', 'vendor', 'field', 'summary'],
    titleField: 'name',
    toggleField: 'published',
    notice:
      'Kolom "Bidang / Program Studi" dan "Penyelenggara" dipakai sebagai penyaring pada halaman publik — tulis konsisten agar pengelompokannya rapi.',
    columns: [
      { name: 'name', label: 'Nama Sertifikat' },
      { name: 'vendor', label: 'Penyelenggara', type: 'badge' },
      { name: 'scheme', label: 'Skema', type: 'badge' },
      { name: 'field', label: 'Bidang' },
      { name: 'exam_fee', label: 'Biaya Ujian', type: 'money' },
      { name: 'published', label: 'Status', type: 'boolean' },
    ],
    fields: [
      { name: 'name', label: 'Nama Sertifikat', type: 'text', required: true, span: 12 },
      { name: 'summary', label: 'Relevansi / Manfaat', type: 'textarea', span: 12, hint: 'Satu kalimat singkat: untuk apa sertifikat ini berguna.' },
      { name: 'field', label: 'Bidang / Program Studi Utama', type: 'text', span: 12, hint: 'Dipakai sebagai penyaring.' },
      { name: 'also_for', label: 'Bidang Lain yang Relevan', type: 'text', span: 12, hint: 'Pisahkan dengan koma. Boleh dikosongkan.' },
      { name: 'vendor', label: 'Penyelenggara / Vendor', type: 'text', span: 12, aside: true, hint: 'Contoh: Microsoft, Cisco, Adobe, PASAS Institute Singapore.' },
      {
        name: 'scheme', label: 'Skema', type: 'select', span: 12, aside: true,
        options: ['internasional', 'nasional'],
        hint: '"internasional" lewat Certiport/PASAS, "nasional" lewat BNSP/LSP.',
      },
      { name: 'exam_fee', label: 'Biaya Ujian (Rp)', type: 'number', span: 12, aside: true, hint: 'Kosongkan bila tidak ingin ditampilkan.' },
      { name: 'priority', label: 'Prioritas', type: 'select', span: 12, aside: true, options: ['Tinggi', 'Sedang', 'Rendah'] },
      { name: 'sort_order', label: 'Urutan Tampil', type: 'number', span: 12, aside: true, hint: 'Angka kecil tampil lebih dulu.' },
      { name: 'published', label: 'Tampilkan', type: 'boolean', span: 12, aside: true },
    ],
  },
  {
    key: 'bukti-sertifikat',
    table: 'certificate_proofs',
    label: 'Bukti Sertifikat',
    labelSingular: 'Bukti Sertifikat',
    description: 'Sertifikat yang sudah diraih tim dan peserta, tampil sebagai bukti kompetensi di halaman Sertifikasi.',
    icon: 'award',
    orderBy: 'sort_order, id',
    searchable: ['title', 'vendor', 'holder'],
    titleField: 'title',
    toggleField: 'published',
    notice:
      'Sertifikat memuat nama pemegang dan nomor verifikasi — keduanya data pribadi. Semua baris di sini sengaja berstatus draf; terbitkan hanya yang pemiliknya sudah setuju ditampilkan ke publik. Nama pemegang boleh dikosongkan bila cukup menampilkan sertifikatnya saja.',
    columns: [
      { name: 'image', label: '', type: 'image', width: 'w-28' },
      { name: 'title', label: 'Nama Sertifikat' },
      { name: 'vendor', label: 'Penerbit', type: 'badge' },
      { name: 'holder', label: 'Pemegang' },
      { name: 'issued_on', label: 'Terbit' },
      { name: 'published', label: 'Status', type: 'boolean' },
    ],
    fields: [
      { name: 'title', label: 'Nama Sertifikat', type: 'text', required: true, span: 12 },
      { name: 'vendor', label: 'Penerbit', type: 'text', span: 12 },
      { name: 'holder', label: 'Nama Pemegang', type: 'text', span: 12, hint: 'Boleh dikosongkan bila tidak ingin menampilkan nama.' },
      { name: 'issued_on', label: 'Tanggal Terbit', type: 'text', span: 6, hint: 'Contoh: 14 Januari 2025.' },
      { name: 'credential_id', label: 'Nomor / Kode Verifikasi', type: 'text', span: 6, hint: 'Boleh dikosongkan.' },
      { name: 'verify_url', label: 'Tautan Verifikasi', type: 'text', span: 12, hint: 'Contoh: https://verify.certiport.com/' },
      { name: 'image', label: 'Gambar Sertifikat', type: 'image', span: 12, aside: true },
      { name: 'sort_order', label: 'Urutan Tampil', type: 'number', span: 12, aside: true },
      { name: 'published', label: 'Tampilkan ke publik', type: 'boolean', span: 12, aside: true },
    ],
  },
  {
    key: 'klien',
